...
export function AddClientModal({ isOpen, onClose }: AddClientModalProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setClient({ ...emptyClient(), password: String(clients.length + 1).padStart(4, '0') });
  ...
  const handleSubmit = async () => {
    ...
    if (!client.client_category_id) newErrors.client_category_id = 'Inserisci una categoria';
    if (!client.phoneNumber) newErrors.phoneNumber = 'Inserisci un numero di telefono';
    if (!client.gender) newErrors.gender = 'Seleziona un genere';
    if (!client.birthDate) {
      newErrors.birthDate = 'Inserisci una data di nascita';
    } else {
      newErrors.birthDate = validateBirthDate(client.birthDate);
    }
    setErrors(newErrors);
        ...
        <div className="max-h-[60vh] overflow-y-auto pr-2">
          <div className="flex flex-col gap-6">
            ...
            <div className="flex flex-row items-start gap-4 w-full">
              ...
              <div className="flex flex-1 flex-col gap-2">
                <label className={labelClass}><Calendar className="size-4 text-zinc-900 dark:text-zinc-100" /><span className="text-sm">Categoria</span></label>
                <CustomSelect
                  options={clientCategories}
                  labelKey="name"
                  valueKey="id"
                  value={client.client_category_id}
                  onChange={(v) => set('client_category_id', v)}
                  placeholder="Seleziona categoria"
                />
                {errors.client_category_id && <p className="text-xs text-red-500">{errors.client_category_id}</p>}
              </div>
