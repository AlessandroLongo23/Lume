 ...
 *
 * For admins the cookie is only trusted when paired with the non-httpOnly
 * `lume-impersonating=1` flag (both set by `/api/platform/enter-salon`).
 * This prevents a stale `lume-active-salon-id` from a prior impersonation
 * from leaking through after the flag cookie is cleared (e.g. browser cookie
 * purge, third-party cookie blockers, logout).
 *
 *   const salonId = await getActiveSalonId(profile.salon_id, profile.role === 'admin');
 */
